...
async function getOrFetchVersions(
  ...
  if (cache) return { versions: cache.versions, source: 'cache' }
...
export async function getAvailableVersions(currentVersion: string): Promise<CliVersionsResponse> {
  const { versions: fetched, source } = await getOrFetchVersions(false)
...
export async function refreshVersionCache(): Promise<{ source: 'github' | 'cache' | 'bundled' }> {
  const { source } = await getOrFetchVersions(true)
  return { source }
}
