...
function renderList() {
  ...
  els.list.innerHTML = credentials
    .map(
      (cred) => `
...
function renderWordSetContent(ws) {
  ...
  const countText =
    searchQuery && filtered.length !== words.length
...
function renderModels() {
  ...
  if (countLabel) {
    ...
    countLabel.textContent =
      n === 0 ? 'No models configured' : n === 1 ? '1 model configured' : `${n} models configured`
  }
  ...
  tbody.innerHTML = fallbackModels
    .map(
      (m) => `
        <tr>
...
function renderTestModelSelect() {
  ...
  if (!select) return
  select.innerHTML = fallbackModels
    .map((m) => `<option value="${esc(m.id)}" ${m.id === modelTestModelId ? 'selected' : ''}>${esc(m.id)}</option>`)
    .join('')
...
async function deleteModel(id) {
  if (
    !(await showConfirm({
      title: 'Delete Model',
      message: `Remove "${id}" from fallback list?`,
      icon: 'trash',
      danger: true,
    }))
  )
